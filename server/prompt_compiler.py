...
SLOT_RULES: List[tuple] = [
    ...
    ("⚠️ 【段落节奏", "纪律", 75), ("⚠️", "纪律", 40),
    # 常驻结构件(当众失态/算账/误读…)是这套引擎**每章必须出现**的东西,
    # 不是泛泛的纪律。实测第7章把它挤了。提到纪律槽最高位。
    ("🧱", "纪律", 92),
    ("#反向提示词库", "纪律", 45),
...
# 纪律槽 1600 实测太紧: 连着三章把主料挤掉(写作纪律/描写配给/常驻结构件),
# 每次都得回头提优先级。1600 是我按「限制尽量少」拍的, 但砍限制该砍在
# **条目数**上(anti_ai_rules 已 12→5), 不该靠挤压随机丢东西。
SLOT_BUDGET = {"任务": 0, "燃料": 2600, "调子": 4200, "纪律": 2200}
...
def measure_text(text: str) -> Dict[str, float]:
    ...
    return {
        ...
        # 全知旁白: 「他不知道的是」「殊不知」—— 上帝视角替读者点破,
        # 通读反馈点名的「AI 写群像戏标准动作」
        "全知旁白": len(re.findall(
            r"不知道的是|殊不知|没有察觉到|并不知道|浑然不知", body)),
    }
...
ENGINE_METRICS: Dict[str, Dict[str, Any]] = {
    ...
    "全知旁白": {
        ...
        "高": "「他不知道的是」「殊不知」是上帝视角替读者点破 —— 删掉；"
             "信息差要靠**读者看得见而角色看不见**的动作与台词呈现，不靠旁白宣布",
    },
    "双问号句": {
        "lo": 0, "hi": 1.0, "item": None,
