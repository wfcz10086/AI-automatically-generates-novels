...
class Novelist:
    ...
    def asset_conflicts(self) -> str:
        ...
        if not hits:
            return ""
        lines = ["\n⚠️ 【设定与红线打架·按这里处理】"]
        # 穿越文的现代专名要**分场景**判, 不能一刀切。实测教训: 我为了防
        # 「纽约混进修真世界」把种子里的纽约/FBI 全替换成架空词, 结果把开局
        # 那场现代世界的追杀戏一起抹掉了 —— 根合同直接从「已经是鼎炉」开始,
        # 读者看着莫名其妙。禁的是**穿越之后**的架空世界, 不是穿越之前。
        lines.append("　⚠ 例外：**主角穿越前那几章（现代世界）照常写现代专名**，"
                     "那是他真实的来处，抹掉了开局就没头没尾。"
                     "下面这条禁令只管**穿越之后的架空世界**。")
    ...
    def audit_account_changes(self, n: int, text: str) -> List[Dict[str, Any]]:
        ...
        r = call("judging",
                 f"只做一件事：数出下面这章里这些物品的**消耗或获得事件**。\n"
                 f"硬账物品：{items}\n\n"
                 f"只数**发生了的动作**（开了几枪、交出/得到了几枚、花了几笔）。\n"
                 f"⚠ 「还剩N发」「剩余N」这类**陈述句不算事件**，绝不许拿它"
                 f"倒推增量 —— 陈述由程序另行对账。拿不准就不计。\n\n"
                 f"—— 正文 ——\n{dst.soft(text, 6000)}\n\n"
    ...
    def era_card(self) -> str:
        ...
        """
        # 架空世界一律不出红线卡 —— 它和种子打架。实测: 红线卡为「纽约」连加
        # 五条同义禁令, 而种子的开局六章**本来就在纽约**(黑帮老大被 FBI 围楼,
        # 然后才穿越)。结果开局那场戏被整段抹掉, 读者看着莫名其妙。
        # 架空世界里「考据」没有权威来源: 一炷香多久是本书自己定的规矩,
        # 不是历史事实, 用一张「那年代没有什么」的卡去管它, 从根上就不成立。
        if (self.p.meta.get("history_mode") or "").lower() == "invented" \
                or self.p.meta.get("era_card_off"):
            return ""
