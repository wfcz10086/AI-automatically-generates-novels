...
#: 口号式收尾。只在**结尾 180 字**内匹配 —— 同样的话出现在章中是人物心声,
#: 出现在结尾就是拿宣言替掉钩子。
SLOGAN_END = re.compile(
    r"(?:这|那)?(?:只是|才)(?:刚刚)?开始"
    r"|在这个[^。！？]{0,14}的世界(?:里|上)"
    r"|他(?:要做的|该做的)[^。！？]{0,20}就是"
    r"|(?:用|靠)自己的方式[^。！？]{0,6}活下去"
    r"|(?:从今往后|从此以后|未来还很长|路还很长)"
    r"|属于(?:他|她|自己)的[^。！？]{0,6}(?:时代|传奇|故事)"
)
...
def audit(text: str, extra_blacklist: List[str] | None = None,
    ...
    # 禁令、没人查; 评审的文风新鲜度给到 84.5 也没抓到 —— 因为它只占全章
    # 一两句, 密度上不显眼。而它偷走的是**章末钩子**: 细纲给了具体钩子,
    # 模型拿口号把它换掉了, 下一章就接不上。
    tail = text.rstrip()[-180:]
    if SLOGAN_END.search(tail):
        issues.append({"level": "high", "type": "口号式收尾",
                       "detail": "结尾用空泛口号／宣言收场，把细纲给的章末钩子换掉了",
                       "sample": tail[-60:]})
