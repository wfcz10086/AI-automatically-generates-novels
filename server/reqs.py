...
在势力名上栽过一次：要把种子里点名的势力钉进势力表，这是 program 档没错，
可我连「哪些词是势力名」也拿正则去套 —— 后缀表 + 虚词表 + 只在某几节里找，
改了四版，每补一条限定词就换一种错法（抠出 39 个垃圾 → 漏掉破折号后面的
相国寺 → 虚词表把太虚院误杀）。

从一段话里认出哪些是势力名，是**语义判断**。program 档说的是「定死」这一步
归程序，不是说内容也要程序去推。正确的分工是三段：

    模型认一次  →  程序存下来  →  之后永远用存的那份

程序在这里只做一件它擅长的事：**字面校验**（模型给的名字必须在种子原文里
逐字出现），挡住它顺手改写。

判断标准：这件事是不是**封闭集合**上的判定。
    自称（洒家/老衲/我…）是封闭词表 → 程序判得了，keep
    哪些词是势力名              是开放语义 → 归模型，程序只存和校验
"""
