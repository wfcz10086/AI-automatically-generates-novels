...
def test_漂移只报最急的几项且方向正确():
    """段落长度正常、但完全不喊也不发问 → 应报叹号/问号那一类偏低。

    topK 从 2 提到至少 3：实测同时有三项明显偏离（段均字数、叹号、对白占比），
    取 2 就永远轮不到第三项 —— 对白占比连着几十章没进过纠偏就是这么来的。
    「只报两项」的原始标定是为了别一次纠太多，但那道保险已经由阻尼（按偏移
    幅度分三档措辞）和 dont_sacrifice（明写别把什么一起砍了）提供了。
    """
    flat = "\n".join(["他点了点头，把账本合上，推到桌子对面去。其实这笔账早就对不上了。"] * 40)
    fb = window_drift([flat] * 6, LLT)
    assert 2 <= fb.count("· ") <= 4, f"报了 {fb.count('· ')} 项，太多或太少"
    assert "偏低" in fb or "不在区间内" in fb
    assert any(k in fb for k in ("叹号", "问号", "独立反问句", "对白")), fb


def test_多数章偏但均值在区间内也要纠():
    """光看均值会漏掉「大多数章都偏、少数章拉回来」这种形态。

    实测对白占比：最近 10 章均值 0.164 稳稳落在 0.16~0.34 内，而逐章看
    **只有 3 章真的在区间内**，另外 7 章在 0.017~0.127 之间 —— 均值是被
    0.26、0.36 那两三章拉进去的。于是这一项 45 章里 30 章偏低，而纠偏机制
    一次都没报过。**均值在区间内 ≠ 这本书在区间内。**
    """
    # 七章几乎没有对白（0.00）+ 三章大段对白（0.79）→ 均值 0.24 稳稳落在
    # 0.16~0.34 区间内，而**十章没有一章真的在区间内**。
    quiet = "\n".join(["他把账本合上，推到桌子对面去，没有说话。"] * 40)
    talky = "\n".join(['“你这账不对。”他说。\n“哪里不对？”\n“处处不对。”'] * 30)
    fb = window_drift([quiet] * 7 + [talky] * 3, LLT)
    assert "对白" in fb, f"多数章偏低却没报出来：\n{fb}"
    assert "不在区间内" in fb, "要把真实理由说出来，不能写成一句自相矛盾的话"
